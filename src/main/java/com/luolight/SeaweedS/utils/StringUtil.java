...
public class StringUtil extends org.apache.commons.lang.StringUtils{
	 ...
	 * @return
	 */
	public static String getBeforeSix(String number){
		if(number.length() <= 6){
			return number;
		}
		return number.substring(number.length() - 6, number.length());
	}
...
/*    public static String xmltoJson(String xml) {
	...
	public static String returnOverDate1(Date time) throws Exception{
		DateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		Date currenttemp = df.parse(df.format(new Date()));
		long between = Math.abs((currenttemp.getTime() - time.getTime()) / 1000);// 除以1000是为了转换成秒
		long day1 = between / (24 * 3600);
		long hour1 = between % (24 * 3600) / 3600;
		long minute1 = between % 3600 / 60;
		if (day1 != 0) {
